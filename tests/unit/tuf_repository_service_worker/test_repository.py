...
class TestMetadataRepository:
    ...
    def test_online_keys_property_from_redis(self, test_repo, monkeypatch):
        ...
        fake_key_obj = pretend.stub(
            from_dict=pretend.call_recorder(
                lambda keyid, key_dict: keys.pop(0)
            )
        )
        monkeypatch.setattr(f"{REPOSITORY_PATH}.Key", fake_key_obj)
    ...
    def test_online_keys_in_memory_override_wins_over_stored_root(
        ...
        stale_root_key = pretend.stub(
            keyid="old_key",
            unrecognized_fields={
                repository.RSTUF_ONLINE_KEY_URI_FIELD: "fn:old"
            },
        )
    ...
    def test__add_delegated_role_keys_no_keys_uses_all_online_keys(
        ...
        targets = pretend.stub(
            signed=pretend.stub(delegations=pretend.stub(keys={}))
        )
        role_metadata = pretend.stub()
        test_repo._sign_with_online_keys = pretend.call_recorder(
            lambda *a, **kw: None
        )
        ...
        assert test_repo._sign_with_online_keys.calls == [
            pretend.call(
                role_metadata, ["online_1", "online_2"], extra_keys={}
            )
        ]
    ...
    def test__add_delegated_role_keys_partial_online_keys_signs_subset(
        ...
        # Role trusts only ONE of two configured online keys, plus an
        # offline key.
        monkeypatch.setattr(
            repository.MetadataRepository,
            "_online_keyids",
    ...
    def test__add_delegated_role_keys_no_role_metadata_skips_signing(
        ...
        # role_metadata=None (used during _update_metadata_delegation)
        # must never sign.
        monkeypatch.setattr(
            repository.MetadataRepository,
            "_online_keyids",
        ...
        test_repo._add_delegated_role_keys(
            targets, delegations, role_name, None
        )
    ...
    def test__sign_with_online_keys_multiple(self, test_repo, monkeypatch):
        ...
        fake_keys = [
            pretend.stub(keyid="keyid1"),
            pretend.stub(keyid="keyid2"),
        ]
        fake_key_obj = pretend.stub(
    ...
    def test__sign_with_online_keys_single(self, test_repo, monkeypatch):
        ...
        assert fake_md.sign.calls == [
            pretend.call("signer_for_keyid", append=True)
        ]

    def test__sign_with_online_keys_keyids_filter(
        self, test_repo, monkeypatch
    ):
    ...
    def test_bump_persist_role_multiple_online_keys(
        self, monkeypatch, test_repo
    ):
        rolename = "test-role"
        ...
        signed_keyids = [call.args[1].keyid for call in test_repo._sign.calls]
    ...
    def test__get_role_for_artifact_path_nested_delegation(self, test_repo):
        ...
        assert test_repo._storage_backend.get.calls == [
            pretend.call(parent_role_name)
        ]
        assert role_metadata.signed.delegations.get_roles_for_target.calls == [
            pretend.call(artifact_path)
        ]

    def test_get_delegation_keyids_nested_hash_bins(
        self, test_repo, monkeypatch
    ):
    ...
    def test__add_metadata_delegation_with_nested_bins(
        self, test_repo, monkeypatch
    ):
        ...
        parent_metadata = Metadata(
            Targets(
                delegations=Delegations(
                    keys={},
                    succinct_roles=pretend.stub(
                        get_roles=lambda: nested_bin_names
                    ),
                )
            )
        )
    ...
    def test__run_online_roles_bump_with_nested_hash_bins(
        self, test_repo, monkeypatch
    ):
        def fake_get_fresh(setting: str):
            if setting == "TARGETS_ONLINE_KEY":
    ...
    def test__resolve_online_keys_role_local_key(self, test_repo, monkeypatch):
        ...
        offline_key = pretend.stub(
            keyid="offline_key_id", unrecognized_fields={}
        )
        monkeypatch.setattr(
            repository.MetadataRepository,
            "_online_keys",
    ...
    def _update_delegation_setup(self, test_repo, monkeypatch, new_keyids):
        ...
        test_repo._update_snapshot = pretend.call_recorder(lambda *a, **kw: 5)
        test_repo._update_timestamp = pretend.call_recorder(
            lambda *a, **kw: None
        )
        return update
